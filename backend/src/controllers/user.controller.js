...
export async function sendFriendRequest(req,res){
    try {
        ...
        if(existingRequest){
            return res
            .status(400)
            .json({message:"A Friend request is already exists between you and this user"});
        }
     ...
     console.error("Error in sendFriendRequest conroller",error.message);
     res.status(500).json({message:"Internal Server Error"});   
    }
}
...
export async function acceptFriendRequest(req,res){
    try {
        ...
        // $addToSet : adds elements to an array only if they do not already exits.
...
export async function getMyFriendRequests(req,res){
    try {
        ...
        console.log("Error in getPendingFrindRequests controller",error.message);
        res.status(500).json({message:"Internal Server Error"});
    }
}
